...
{
	globalThis.C3.Plugins.StraniAnelli_InjectCSS.Cnds =
	{
		...
		/*async IsCSSAlreadyPresent(nameFile) {
			 await this._triggerAsync(globalThis.C3.Plugins.StraniAnelli_InjectCSS.Cnds.IsCSSAlreadyPresentVerified)
	 	},*/
		/*IsCSSAlreadyPresent(nomeCSS) {

			let urlCSSinC3 = '';
			if ( this.runtime.IsPreview() ) {
				urlCSSinC3 = this.runtime.GetAssetManager().GetProjectFileUrl(nomeCSS);
			}  else {
				urlCSSinC3 = nomeCSS;
			}
		...
		// async IsCSSAlreadyPresent(nameFile) {
		// 	let url = await this.runtime.GetAssetManager().LoadProjectFileUrl(nameFile);
		// 	const links = document.head.getElementsByTagName("link");
    // 	for(let i = 0; i < links.length; i++) {
    ...
    // 		}
		...
		// IsCSSAlreadyPresent(nameFile) {
		// 	this.runtime.GetAssetManager().LoadProjectFileUrl( nameFile ).then(url=>
		// 	{
		// 		const links = document.head.getElementsByTagName("link");
		// 		for(let i = 0; i < links.length; i++) {
		//         if (links[i].href.substr(-nameFile.length) == nameFile) return true;
